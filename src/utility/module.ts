import { Rule, Tree, SchematicContext, DirEntry, SchematicsException } from "@angular-devkit/schematics";
...
export function addDeclarationToNgModule(options: any): Rule {
    return (tree: Tree, _context: SchematicContext) => {
        ...
        const componentPath = `${options.path}/` + strings.dasherize(options.name) + '/' + strings.dasherize(options.name) + '.component';    
...
function findModule(tree:Tree, options: any): Path {
    ...
    while(dir) {
        ...
        if (filteredMatches.length == 1) {
            ...
            throw new SchematicsException('Es wurde mehr als ein Modul gefunden. Verwenden Sie die Option "importToNgModule", ' +
    ...
    throw new SchematicsException('Kein Modul gefunden. Verwenden Sie die Option "importToNgModule" um den Import zu überspringen.');
}
